...
(function() {
	...
	function IosSelect(level, data, options) {
		...
		this.options.parentIdName = options.parentIdName || 'parentId';
		this.options.idName = options.idName || 'id';
		this.options.valueName = options.valueName || 'value';
	...
	IosSelect.prototype = {
		...
		initLayer: function() {
			...
			this.scrollOne.on('scrollEnd', function() {
				...
				if (self.level > 1 && self.options.oneTwoRelation === 1) {
					self.setTwoLevel(self.selectOneObj[self.options.idName.toLowerCase()], self.selectTwoObj[self.options.idName], self.selectThreeObj[self.options.idName.toLowerCase()], self.selectFourObj[self.options.idName.toLowerCase()], self.selectFiveObj[self.options.idName.toLowerCase()]);
				}
			});
			...
			if (this.level >= 2) {
				...
				this.scrollTwo.on('scrollEnd', function() {
					...
					if (self.level > 2 && self.options.twoThreeRelation === 1) {
						self.setThreeLevel(self.selectOneObj[self.options.idName.toLowerCase()], self.selectTwoObj[self.options.idName.toLowerCase()], self.selectThreeObj[self.options.idName.toLowerCase()], self.selectFourObj[self.options.idName.toLowerCase()], self.selectFiveObj[self.options.idName.toLowerCase()]);
					}
				});
                this.scrollTwo.on('scrollCancel', function() {
					...
					if (self.level > 2 && self.options.twoThreeRelation === 1) {
						self.setThreeLevel(self.selectOneObj[self.options.idName.toLowerCase()], self.selectTwoObj[self.options.idName], self.selectThreeObj[self.options.idName.toLowerCase()], self.selectFourObj[self.options.idName.toLowerCase()], self.selectFiveObj[self.options.idName.toLowerCase()]);
					}
				});
			}
			if (this.level >= 3) {
				...
				this.scrollThree.on('scrollEnd', function() {
					...
					if (self.level >= 4 && self.options.threeFourRelation === 1) {
						self.setFourLevel(self.selectOneObj[self.options.idName.toLowerCase()], self.selectTwoObj[self.options.idName], self.selectThreeObj[self.options.idName.toLowerCase()], self.selectFourObj[self.options.idName.toLowerCase()], self.selectFiveObj[self.options.idName.toLowerCase()]);
					}
				});
			...
			if (this.level >= 4) {
				...
				this.scrollFour.on('scrollEnd', function() {
					...
					if (self.level >= 5 && self.options.fourFiveRelation === 1) {
						self.setFiveLevel(self.selectOneObj[self.options.idName.toLowerCase()], self.selectTwoObj[self.options.idName], self.selectThreeObj[self.options.idName.toLowerCase()], self.selectFourObj[self.options.idName.toLowerCase()], self.selectFiveObj[self.options.idName.toLowerCase()]);
					}
				});
		...
		renderOneLevel: function(oneLevelId, twoLevelId, threeLevelId, fourLevelId, fiveLevelId, oneLevelData) {
			...
			var hasAtId = oneLevelData.some(function(v, i, o) {
				return v[self.options.idName] == oneLevelId;
			});
			if (!hasAtId) {
				oneLevelId = oneLevelData[0][this.options.idName];
			}
			var oneHtml = '';
			var self = this;
			var plast = 0;
			...
			oneLevelData.forEach(function(v, i, o) {
				if (v[self.options.idName] == oneLevelId) {
					oneHtml += '<li style="height: ' + this.options.itemHeight + this.options.cssUnit + '; line-height: ' + this.options.itemHeight + this.options.cssUnit +';" ' + iosSelectUtil.attrToHtml(v) + ' class="at">' + v[self.options.valueName] + '</li>';
					plast = i + 1;
				} else {
					oneHtml += '<li style="height: ' + this.options.itemHeight + this.options.cssUnit + '; line-height: ' + this.options.itemHeight + this.options.cssUnit +';"' + iosSelectUtil.attrToHtml(v) + '>' + v[self.options.valueName] + '</li>';
				}
			}.bind(this));
			oneHtml += this.getWhiteItem();
		...
		getTwoLevel: function(oneLevelId) {
			...
			if (this.options.oneTwoRelation === 1) {
				this.data[1].forEach(function(v, i, o) {
					if (v[self.options.parentIdName] == oneLevelId) {
						twoLevelData.push(v);
					}
				});
			} else {
		...
		renderTwoLevel: function(oneLevelId, twoLevelId, threeLevelId, fourLevelId, fiveLevelId, twoLevelData) {
			...
			var hasAtId = twoLevelData.some(function(v, i, o) {
				return v[self.options.idName] == twoLevelId;
			});
			if (!hasAtId) {
				twoLevelId = twoLevelData[0][this.options.idName];
			}
			var twoHtml = '';
			var self = this;
			twoHtml += this.getWhiteItem();
			twoLevelData.forEach(function(v, i, o) {
				if (v[self.options.idName] == twoLevelId) {
					twoHtml += '<li style="height: ' + this.options.itemHeight + this.options.cssUnit + '; line-height: ' + this.options.itemHeight + this.options.cssUnit +';"' + iosSelectUtil.attrToHtml(v) + ' class="at">' + v[self.options.valueName] + '</li>';
					plast = i + 1;
				} else {
					twoHtml += '<li style="height: ' + this.options.itemHeight + this.options.cssUnit + '; line-height: ' + this.options.itemHeight + this.options.cssUnit +';"' + iosSelectUtil.attrToHtml(v) + '>' + v[self.options.valueName] + '</li>';
				}
			}.bind(this));
			twoHtml += this.getWhiteItem();
		...
		getThreeLevel: function(oneLevelId, twoLevelId) {
			...
			if (this.options.twoThreeRelation === 1) {
				this.data[2].forEach(function(v, i, o) {
					if (v[self.options.parentIdName] == twoLevelId) {
	    ...
	    renderThreeLevel: function(oneLevelId, twoLevelId, threeLevelId, fourLevelId, fiveLevelId, threeLevelData) {
			...
			var hasAtId = threeLevelData.some(function(v, i, o) {
				return v[self.options.idName] == threeLevelId;
			});
			if (!hasAtId) {
				threeLevelId = threeLevelData[0][this.options.idName];
			}
			var threeHtml = '';
			var self = this;
			threeHtml += this.getWhiteItem();
			threeLevelData.forEach(function(v, i, o) {
				if (v[self.options.idName] == threeLevelId) {
					threeHtml += '<li style="height: ' + this.options.itemHeight + this.options.cssUnit + '; line-height: ' + this.options.itemHeight + this.options.cssUnit +';"' + iosSelectUtil.attrToHtml(v) + ' class="at">' + v[self.options.valueName] + '</li>';
					plast = i + 1;
				} else {
					threeHtml += '<li style="height: ' + this.options.itemHeight + this.options.cssUnit + '; line-height: ' + this.options.itemHeight + this.options.cssUnit +';"' + iosSelectUtil.attrToHtml(v) + '>' + v[self.options.valueName] + '</li>';
				}
			}.bind(this));
			threeHtml += this.getWhiteItem();
	    ...
	    getFourLevel: function(oneLevelId, twoLevelId, threeLevelId) {
			...
			if (this.options.threeFourRelation === 1) {
				this.data[3].forEach(function(v, i, o) {
					if (v[self.options.parentIdName] == threeLevelId) {
	    ...
	    renderFourLevel: function(oneLevelId, twoLevelId, threeLevelId, fourLevelId, fiveLevelId, fourLevelData) {
			...
			var hasAtId = fourLevelData.some(function(v, i, o) {
				return v[self.options.idName] == fourLevelId;
			});
			if (!hasAtId) {
				fourLevelId = fourLevelData[0][this.options.idName];
			}
			var fourHtml = '';
			var self = this;
			fourHtml += this.getWhiteItem();
			fourLevelData.forEach(function(v, i, o) {
				if (v[self.options.idName] == fourLevelId) {
					fourHtml += '<li style="height: ' + this.options.itemHeight + this.options.cssUnit + '; line-height: ' + this.options.itemHeight + this.options.cssUnit +';"' + iosSelectUtil.attrToHtml(v) + ' class="at">' + v[self.options.valueName] + '</li>';
					plast = i + 1;
				} else {
					fourHtml += '<li style="height: ' + this.options.itemHeight + this.options.cssUnit + '; line-height: ' + this.options.itemHeight + this.options.cssUnit +';"' + iosSelectUtil.attrToHtml(v) + '>' + v[self.options.valueName] + '</li>';
				}
			}.bind(this));
			fourHtml += this.getWhiteItem();
	    ...
	    getFiveLevel: function(oneLevelId, twoLevelId, threeLevelId, fourLevelId) {
			...
			if (this.options.fourFiveRelation === 1) {
				this.data[4].forEach(function(v, i, o) {
					if (v[self.options.parentIdName] == fourLevelId) {
	    ...
	    renderFiveLevel: function(oneLevelId, twoLevelId, threeLevelId, fourLevelId, fiveLevelId, fiveLevelData) {
			...
			var hasAtId = fiveLevelData.some(function(v, i, o) {
				return v[self.options.idName] == fiveLevelId;
			});
			if (!hasAtId) {
				fiveLevelId = fiveLevelData[0][this.options.idName];
			}
			var fiveHtml = '';
			var self = this;
			fiveHtml += this.getWhiteItem();
			fiveLevelData.forEach(function(v, i, o) {
				if (v[self.options.idName] == fiveLevelId) {
					fiveHtml += '<li style="height: ' + this.options.itemHeight + this.options.cssUnit + '; line-height: ' + this.options.itemHeight + this.options.cssUnit +';"' + iosSelectUtil.attrToHtml(v) + ' class="at">' + v[self.options.valueName] + '</li>';
					plast = i + 1;
				} else {
					fiveHtml += '<li style="height: ' + this.options.itemHeight + this.options.cssUnit + '; line-height: ' + this.options.itemHeight + this.options.cssUnit +';"' + iosSelectUtil.attrToHtml(v) + '>' + v[self.options.valueName] + '</li>';
				}
			}.bind(this));
			fiveHtml += this.getWhiteItem();
