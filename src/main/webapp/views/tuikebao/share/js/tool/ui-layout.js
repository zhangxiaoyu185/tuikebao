Views.PanelView = {
	...
	show: function(param, isBackPage) {
		...
		if(self.options.hasFootNav) {
			$('.footNavBar_item .'+self.options.itemClass).removeClass('selected');
			$('.footNavBar_item .'+self.options.itemClass).eq(self.options.footItemOrder).addClass('selected');
			$("#placeholder").show();
			$("#footNavBar").show();
		}else {
			$("#placeholder").hide();
	...
	scrollInit: function(tmplName, sourceData) {
		// if(tmplName == 'indexItem'){
		// 	var li = $('#scroller').find('#scroller-content').find('ul.index_page').find('li.notRefresh');
		// 	$('#scroller').find('#scroller-content').find('ul.index_page').html('');
		...
		$('#scroller').find('#scroller-content').find('ul').eq(0).append(li);
		// }
		// var li = $('#scroller').find('#scroller-content').find('ul').eq(0).find('.notRefresh');
		...
		var ulEls = $('#scroller').find('#scroller-content').find('ul');//.append(listContent);
		for(var i=0; i<ulEls.length; i++) {//解决  上啦刷新下拉加载和轮播图的冲突
			if(!$(ulEls[i]).hasClass('slidesjs-pagination')&&!$(ulEls[i]).hasClass('h_scroller_ul')){
...
function changeURL(stateJson,url){
	...
	window.onpopstate = function(){
	  ...
	  // 	if(pageName=='index'||pageName=='hotList'||pageName=='inviteFriends'||pageName=='serviceIndex'||pageName=='myIndex'){  
	  //       var baseUrl = window.location.href.split('?');
			// 	baseUrl = baseUrl.length==2 ? baseUrl[0] : baseUrl;
