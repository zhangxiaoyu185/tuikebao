...
Views.serviceIndexView =  $.extend({}, Views.PanelView, {
    ...
    willShow: function(param, isBackPage) {
       var self = this;
        var url = WEB_URL + "busiServerType/share/find/all";
    ...
    goList: function(btn) {
        ...
        if(list[index].bssteIslist==0) {
            ...
            dataSave('serviceListUuid', bssteUuid);
            Views.serviceListView.willShow();
        }
...
Views.serviceListView =  $.extend({}, Views.PanelView, {
    ...
    getServiceList: function(type,pageNum,pageSize) {
        ...
        var data = {
            bssdcType: dataGet('serviceListUuid'),
            pageNum: pageNum,
            pageSize: pageSize
        };
    ...
    goDesc:function(btn) {
        var bssdcUuid = $(btn).attr('data-bssteUuid');
        Views.serviceDescView.willShow(bssdcUuid);
    }  
});
...
Views.onLineView =  $.extend({}, Views.PanelView, {
    ...
    willShow: function(param, isBackPage) {
        var self = this;
        var data = {crcseUuid:"161126131423aaaa"};
        var url = WEB_URL + "coreCustomerService/views";
        ajaxTool("post", data, url,
            function error(XMLHttpRequest, textStatus, errorThrown,fnErr){
                alert("error:请求失败");
            },
            function success(data){
                if(!data.success) {
                    ...
                    self.crcseTel =data.data.crcseTel;
                    self.crcseQq =data.data.crcseQq;
                    var telphone =self.crcseTel.split("|");
                    var customerQq=self.crcseQq.split("|");
                    self.show({crcseTel:data.data.crcseTel,telphone:telphone,customerQq:customerQq}, isBackPage);
...
/***********************关于我们**********************/
Views.aboutUsView =  $.extend({}, Views.PanelView, {
    options: {
        tmpl: 'aboutUs'
    },
