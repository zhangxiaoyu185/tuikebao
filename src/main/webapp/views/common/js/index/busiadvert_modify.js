...
function getInfo(id){
	...
	getOData(str,"busiAdvert/views",{fn:function(oData){
		if(oData.code == 1) {
			...
			$(".bsaetOrd").val(oData.data.bsaetOrd || "");
			$(".bsaetDesc").val(oData.data.bsaetDesc || "");
			
			upImgItem.changeShow('img_upload_pic', 'bsaetPic', oData.data.bsaetPic, 750, 200, '12', '12', '750X200', function(widget) {
...
function Modify(msgObject){
	...
	var str = 'bsaetUuid='+encodeURIComponent(bsaetUuid)+'&bsaetLink='+encodeURIComponent(bsaetLink)+'&bsaetPic='+encodeURIComponent(bsaetPic)+'&bsaetOrd='+encodeURIComponent(bsaetOrd)+'&bsaetDesc='+encodeURIComponent(bsaetDesc);
	getOData(str,"busiAdvert/update/busiAdvert",{
		fn:function(oData){
