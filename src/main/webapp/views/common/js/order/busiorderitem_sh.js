...
$(function () {
	...
	$("body").delegate('.refund','click', function(){
		layer.open({
			...
			shadeClose: false, //点击遮罩关闭层
			area : [widthLayer , heightLayer],
			content: '../order/busiorderitem_modify_tk.html?id='+$(this).attr("data-id")+'&timestamp='+(new Date()).valueOf()
		});
	});
	//完成
	$("body").delegate('.finish','click', function(){
		layer.open({
			...
			shadeClose: false, //点击遮罩关闭层
			area : [widthLayer , heightLayer],
			content: '../order/busiorderitem_modify_bytk.html?id='+$(this).attr("data-id")+'&timestamp='+(new Date()).valueOf()
		});
	});
	//全选
...
function showList(obj,pagenum){
	...
	var str = '';
	if(obj.bsoimUuid==""){
		str = 'pageNum='+pagenum+'&pageSize=10&bsoimStatus=9';
	}else{
		str = 'pageNum='+pagenum+'&pageSize=10&bsoimStatus=9&bsoimUuid='+encodeURIComponent(obj.bsoimUuid);
	}
	getOData(str,"busiOrder/item/find/by/cnd/my/status/back",{fn:function(oData){
		...
		for(var i=0;i<ln;i++){
			strhtml_list += '<tr class="trHighLight">'
				...
				if (arrData[i].bsoimExpressCode !=null && arrData[i].bsoimExpno != null){
					strhtml_list += '<a  class="p-edit express" data-id="'+arrData[i].bsoimUuid+'"  data-code="'
					+arrData[i].bsoimExpressCode+'"   data-no="'+arrData[i].bsoimExpno+'">查看物流</a>'
				} 
			    strhtml_list +='<a  class="p-edit refund" data-id="'+arrData[i].bsoimUuid+'">退款</a>'
				+'<a  class="p-edit finish" data-id="'+arrData[i].bsoimUuid+'">完成</a>'
				+'</td>'
				+'</tr>';
		}
