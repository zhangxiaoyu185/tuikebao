...
$(function () {
	...
	$("body").delegate('.modifyit','click', function(){
		layer.open({
			...
			shadeClose: false, //点击遮罩关闭层
			area : [widthLayer , heightLayer],
			content: '../order/busiorderitem_modify_fh.html?id='+$(this).attr("data-id")+'&timestamp='+(new Date()).valueOf()
		});
	});
	//转为售后
...
function showList(obj,pagenum){
	...
	var str = '';
	if(obj.bsoimUuid==""){
		str = 'pageNum='+pagenum+'&pageSize=10&bsoimStatus=2';
	}else{
		str = 'pageNum='+pagenum+'&pageSize=10&bsoimStatus=2&bsoimUuid='+encodeURIComponent(obj.bsoimUuid);
	}
	getOData(str,"busiOrder/item/find/by/cnd/my/status/back",{fn:function(oData){
		...
		for(var i=0;i<ln;i++){
			strhtml_list += '<tr class="trHighLight">'
				...
				+'<a  class="p-edit modifyit" data-id="'+arrData[i].bsoimUuid+'">发货</a>'
