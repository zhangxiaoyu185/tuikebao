...
$(function () {
	...
	//查看评价
	$("body").delegate('.evaluation','click', function(){
		layer.open({
			type: 2,
			title: '详情',
			scrollbar: false,
...
function showList(obj,pagenum){
	...
	var str = '';
	if(obj.bsoimUuid==""){
		str = 'pageNum='+pagenum+'&pageSize=10&bsoimStatus=8';
	}else{
		str = 'pageNum='+pagenum+'&pageSize=10&bsoimStatus=8&bsoimUuid='+encodeURIComponent(obj.bsoimUuid);
	}
	getOData(str,"busiOrder/item/find/by/cnd/my/status/back",{fn:function(oData){
		...
		for(var i=0;i<ln;i++){
			strhtml_list += '<tr class="trHighLight">'
				...
				+'<a  class="p-edit evaluation" data-id="'+arrData[i].bsoimUuid+'">查看评价</a>'  
				+'</td>'
				+'</tr>';
		}
