...
$(function () {
	...
	$("body").delegate('.detailit','click', function(){
		layer.open({
			...
			content: '../order/busiorderexpress_detail.html?id='+$(this).attr("data-id")+'&timestamp='+(new Date()).valueOf()
		});
	});
	//新增
	$('.addit').on('click', function(){
		layer.open({
			...
			shadeClose: false, //点击遮罩关闭层
			area : [widthLayer , heightLayer],
			content: '../order/busiorderexpress_add.html?timestamp='+(new Date()).valueOf()
		});
	});
	//修改
	$("body").delegate('.modifyit','click', function(){
		layer.open({
			...
			shadeClose: false, //点击遮罩关闭层
			area : [widthLayer , heightLayer],
			content: '../order/busiorderexpress_modify.html?id='+$(this).attr("data-id")+'&timestamp='+(new Date()).valueOf()
		});
	});
	//全选
	...
	$("body").delegate(".delthese","click",function(){
		...
		if(ids == ""){
			...
			if (r==true){
				var str = 'bsoesUuids='+encodeURIComponent(ids);
				getOData(str,"busiOrderExpress/delete/batch",{fn:function(oData){
						var obj = {};//查询条件对象
						searchContent(obj);
	...
	$("body").delegate(".delit","click",function(){
		...
		if (r==true){
			var str = 'bsoesUuid='+encodeURIComponent($(this).attr("data-id"));
			getOData(str,"busiOrderExpress/delete/one",{fn:function(oData){
					var obj = {};//查询条件对象
					searchContent(obj);
					pagenum = parseInt($(".curpage").text());
					isNull(obj,pagenum);
...
function showList(obj,pagenum){
	var aData =[{name:"<input type='checkbox' name='checkboxAll' value='checkbox' />",percent:"5"},
				...
				{name:"物流内容",percent:"10"},
				{name:"添加时间",percent:"10"},
				{name:"操作",percent:"10"}];
	setTableHead(aData);
	var str = '';
	if(obj.crcseTitle==""){
	...
	getOData(str,"busiOrderExpress/find/by/cnd",{fn:function(oData){
		var strhtml_list = "";
		var arrData = oData.data.result;
		var ln = arrData.length;
		for(var i=0;i<ln;i++){
			strhtml_list += '<tr class="trHighLight">'
				+'<td>'+'<input type="checkbox" name="checkbox" value="checkbox" data-id="'+arrData[i].bsoesUuid+'"/>'+'</td>'
				+'<td>'+(arrData[i].bsoesOrderItem || "")+'</td>'
				+'<td>'+(arrData[i].bsoesContent || "")+'</td>'
				+'<td>'+(arrData[i].bsoesCdate || "")+'</td>'
				+'<td>'
				+'<a  class="p-edit detailit" data-id="'+arrData[i].bsoesUuid+'">查看</a>'
				+'<a  class="p-edit modifyit" data-id="'+arrData[i].bsoesUuid+'">修改</a>'
				+'<a  class="p-edit delit" data-id="'+arrData[i].bsoesUuid+'">删除</a>'
				+'</td>'
				+'</tr>';
		}
