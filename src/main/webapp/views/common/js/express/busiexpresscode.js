...
$(function () {
	...
	$("body").delegate('.detailit','click', function(){
		layer.open({
			...
			content: '../express/busiexpresscode_detail.html?id='+$(this).attr("data-id")+'&timestamp='+(new Date()).valueOf()
		});
	});
	//新增
	$('.addit').on('click', function(){
		layer.open({
			...
			shadeClose: false, //点击遮罩关闭层
			area : [widthLayer , heightLayer],
			content: '../express/busiexpresscode_add.html?timestamp='+(new Date()).valueOf()
		});
	});
	//修改
	$("body").delegate('.modifyit','click', function(){
		layer.open({
			...
			shadeClose: false, //点击遮罩关闭层
			area : [widthLayer , heightLayer],
			content: '../express/busiexpresscode_modify.html?id='+$(this).attr("data-id")+'&timestamp='+(new Date()).valueOf()
		});
	});
	//全选
	...
	$("body").delegate(".delthese","click",function(){
		...
		if(ids == ""){
			...
			if (r==true){
				var str = 'bseceUuids='+encodeURIComponent(ids);
				getOData(str,"busiExpressCode/delete/batch",{fn:function(oData){
						var obj = {};//查询条件对象
						searchContent(obj);
	...
	$("body").delegate(".delit","click",function(){
		...
		if (r==true){
			var str = 'bseceUuid='+encodeURIComponent($(this).attr("data-id"));
			getOData(str,"busiExpressCode/delete/one",{fn:function(oData){
					var obj = {};//查询条件对象
					searchContent(obj);
					pagenum = parseInt($(".curpage").text());
					isNull(obj,pagenum);
...
function showList(obj,pagenum){
	...
	getOData(str,"busiExpressCode/find/by/cnd",{fn:function(oData){
		var strhtml_list = "";
		var arrData = oData.data.result;
		var ln = arrData.length;
		for(var i=0;i<ln;i++){
			strhtml_list += '<tr class="trHighLight">'
				+'<td>'+'<input type="checkbox" name="checkbox" value="checkbox" data-id="'+arrData[i].bseceUuid+'"/>'+'</td>'
				+'<td>'+(arrData[i].bseceCode || "")+'</td>'
				+'<td>'+(arrData[i].bseceName || "")+'</td>'
				+'<td>'
				+'<a  class="p-edit detailit" data-id="'+arrData[i].bseceUuid+'">查看</a>'
				+'<a  class="p-edit modifyit" data-id="'+arrData[i].bseceUuid+'">修改</a>'
				+'<a  class="p-edit delit" data-id="'+arrData[i].bseceUuid+'">删除</a>'
				+'</td>'
				+'</tr>';
		}
