...
$(function () {
	...
	$("body").delegate('.detailit','click', function(){
		layer.open({
			...
			content: '../my/busipromoterinfo_detail.html?id='+$(this).attr("data-id")+'&timestamp='+(new Date()).valueOf()
		});
	});
...
function showList(obj,pagenum){
	var aData =[{name:"<input type='checkbox' name='checkboxAll' value='checkbox' />",percent:"5"},
				...
				{name:"支付宝账户",percent:"30"},
				{name:"支付宝姓名",percent:"25"},
				{name:"操作",percent:"20"}];
	setTableHead(aData);
	var str = '';
	if(obj.userName==""){
	...
	getOData(str,"busiPromoterInfo/find/by/cnd",{fn:function(oData){
		var strhtml_list = "";
		var arrData = oData.data.result;
		var ln = arrData.length;
		for(var i=0;i<ln;i++){
			strhtml_list += '<tr class="trHighLight">'
				+'<td>'+'<input type="checkbox" name="checkbox" value="checkbox" data-id="'+arrData[i].bspioUuid+'"/>'+'</td>'
				+'<td>'+(arrData[i].bspioUserName || "")+'</td>'
				+'<td>'+(arrData[i].bspioAlipayNo || "")+'</td>'
				+'<td>'+(arrData[i].bspioAlipayName || "")+'</td>'
				+'<td>'
				+'<a  class="p-edit detailit" data-id="'+arrData[i].bspioUuid+'">查看</a>'
				+'</td>'
				+'</tr>';
		}
