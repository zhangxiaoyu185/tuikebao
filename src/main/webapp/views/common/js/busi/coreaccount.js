...
$(function () {
	...
	$("body").delegate('.modifypsd','click', function(){
		...
		if (r==true){
			...
			getOData(str,"coreAccount/reset/pwd",{fn:function(oData){
					alert("重置密码为123456！");
				}}
			);
		}
	...
	$("body").delegate(".delthese","click",function(){
		...
		if(ids == ""){
			...
			if (r==true){
				...
				getOData(str,"coreAccount/delete/batch",{fn:function(oData){
						var obj = {};//查询条件对象
						searchContent(obj);
	...
	$("body").delegate(".delit","click",function(){
		...
		if (r==true){
			...
			getOData(str,"coreAccount/delete/one",{fn:function(oData){
					var obj = {};//查询条件对象
					searchContent(obj);
					pagenum = parseInt($(".curpage").text());
					isNull(obj,pagenum);
...
function showList(obj,pagenum){
	...
	getOData(str,"coreAccount/find/by/cnd",{fn:function(oData){
		var strhtml_list = "";
