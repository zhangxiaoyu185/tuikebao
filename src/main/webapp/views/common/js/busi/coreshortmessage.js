...
$(function () {
	...
	$("body").delegate('.detailit','click', function(){
		layer.open({
			...
			content: '../busi/coreshortmessage_detail.html?id='+$(this).attr("data-id")+'&timestamp='+(new Date()).valueOf()
		});
	});
	//全选
...
/**
	...
	$("body").delegate(".delthese","click",function(){
		...
		if(ids == ""){
			...
			if (r==true){
				var str = 'crmceUserName='+encodeURIComponent(ids);
				getOData(str,"coreShortMessage/delete/batch",{fn:function(oData){
						var obj = {};//查询条件对象
						searchContent(obj);
	...
	$("body").delegate(".delit","click",function(){
		...
		if (r==true){
			var str = 'crmceUserName='+encodeURIComponent($(this).attr("data-id"));
			getOData(str,"coreShortMessage/delete/one",{fn:function(oData){
					var obj = {};//查询条件对象
					searchContent(obj);
					pagenum = parseInt($(".curpage").text());
					isNull(obj,pagenum);
...
function showList(obj,pagenum){
	...
	getOData(str,"coreShortMessage/find/by/cnd",{fn:function(oData){
		var strhtml_list = "";
		...
		for(var i=0;i<ln;i++){
			...
			strhtml_list += '<tr class="trHighLight">'
				...
				+'<td>'+(arrData[i].crmceTime || "")+'</td>'
				+'<td>'+(arrData[i].crmceTimeout || "")+'</td>'
				+'<td>'+(crmceStatusCH || "")+'</td>'
				+'<td>'
