...
$(function () {
	...
	$("body").delegate('.detailit','click', function(){
		layer.open({
			...
			content: '../busi/coresystemconfig_detail.html?id='+$(this).attr("data-id")+'&timestamp='+(new Date()).valueOf()
		});
	});
	//新增
	$('.addit').on('click', function(){
		layer.open({
			...
			shadeClose: false, //点击遮罩关闭层
			area : [widthLayer , heightLayer],
			content: '../busi/coresystemconfig_add.html?timestamp='+(new Date()).valueOf()
		});
	});
	//修改
	$("body").delegate('.modifyit','click', function(){
		layer.open({
			...
			shadeClose: false, //点击遮罩关闭层
			area : [widthLayer , heightLayer],
			content: '../busi/coresystemconfig_modify.html?id='+$(this).attr("data-id")+'&timestamp='+(new Date()).valueOf()
		});
	});
	//全选
	...
	$("body").delegate(".delthese","click",function(){
		...
		if(ids == ""){
			...
			if (r==true){
				...
				getOData(str,"coreSystemConfig/delete/batch",{fn:function(oData){
						var obj = {};//查询条件对象
						searchContent(obj);
	...
	$("body").delegate(".delit","click",function(){
		...
		if (r==true){
			...
			getOData(str,"coreSystemConfig/delete/one",{fn:function(oData){
					var obj = {};//查询条件对象
					searchContent(obj);
					pagenum = parseInt($(".curpage").text());
					isNull(obj,pagenum);
...
function showList(obj,pagenum){
	...
	getOData(str,"coreSystemConfig/find/by/cnd",{fn:function(oData){
		var strhtml_list = "";
