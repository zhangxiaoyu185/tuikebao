...
public class ExcelUtils {
    ...
    private static String buildExcelFileName(String prefix) {
        SimpleDateFormat formatter = new SimpleDateFormat("yyyyMMddHHmmss");
        String dateString = formatter.format(new Date());
        return (new StringBuilder(String.valueOf(prefix))).append('_').append(dateString).append(".xlsx").toString();
    }
    ...
    public static void writeXls(HttpServletResponse response, List<String[]> contentList, String fileNamePrefix) throws IOException {
        if(response == null || contentList == null || StringUtils.isBlank(fileNamePrefix)) {
            ...
            response.setHeader("Content-Disposition", (new StringBuilder("attachment;filename=")).append(buildExcelFileName(fileNamePrefix)).toString());
